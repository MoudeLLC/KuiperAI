#!/usr/bin/env python3
"""
HARDER Autoregressive Training
Uses combined dataset with 2,084 examples (vs 1,311 before)
"""
import sys
...
print("=" * 70)
print("HARDER AUTOREGRESSIVE TRAINING")
print("More data + More complex examples = Better AI")
print("=" * 70)

# Load COMBINED training data
print("\n[1/6] Loading HARDER training data...")
with open('knowledge/combined_training_dataset.txt', 'r') as f:
    texts = [line.strip() for line in f if line.strip()]

print(f"  ✓ Loaded {len(texts)} training examples (was 1,311)")
...
# Use /kaggle/working if in Kaggle
import os
...
for epoch in range(epochs):
    ...
    if avg_loss < best_loss:
        best_loss = avg_loss
        model.save(f'{checkpoint_dir}/harder_model_best.pt')
        print(f"  ✓ Saved best model (loss: {best_loss:.4f})")

print("\n" + "=" * 70)
print("HARDER TRAINING COMPLETE!")
print("=" * 70)
print(f"Best Loss: {best_loss:.4f}")
print(f"Model saved to: {checkpoint_dir}/harder_model_best.pt")
print(f"Vocabulary saved to: {checkpoint_dir}/vocab_harder.json")

# Save vocabulary
import json
with open(f'{checkpoint_dir}/vocab_harder.json', 'w') as f:
    json.dump(vocab, f, indent=2)

print("\n✓ Ready for generation!")
print("✓ More data + harder examples = smarter AI!")
